...
void SortedMergeRecur(Node*& xHead, Node*& yHead, Node*& zHead)
{
   ...
   // If the X-List and Y-List are empty, return nothing.
   if (xHead == nullptr && yHead == nullptr)
      return;

   // If the Y-List is not empty AND (the X-List is empty OR yHead data
   // is less than or equal to xHead data (data: integer values)), then
   // populate and sort the Z-List in non-decreasing order with respect to
   // the Y-List.)
   if (yHead != nullptr && (xHead == nullptr || yHead->data <= xHead->data))
   {
      zHead = yHead;
      yHead = yHead->link;
      zHead->link = nullptr;
   }
   else // Otherwise, populate and sort the Z-List in non-decreasing order
        // with respect to the X-List.
   {
      zHead = xHead;
      xHead = xHead->link;
      zHead->link = nullptr;
   }
   SortedMergeRecur(xHead, yHead, zHead->link);  // Do this again until
                                                 // the Z-List is fully
                                                 // populated and sorted
                                                 // with X-List and Y-List
                                                 // data fields and X-List
                                                 // and Y-List are empty.
}
