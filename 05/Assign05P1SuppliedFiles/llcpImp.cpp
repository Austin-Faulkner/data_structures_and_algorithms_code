...
void PromoteTarget(Node*& headPtr, int target)
{
   ...
   if (!headPtr) // If there is not a headPtr, create one with target.
   {
      headPtr = newNode;
         return;
   }

   if (headPtr->link == nullptr) // It there is a single node . . .
   {
      if (headPtr->data != target) // . . . and it is not the target,
