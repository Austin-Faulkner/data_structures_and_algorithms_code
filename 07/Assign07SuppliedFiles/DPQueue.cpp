...
namespace CS3358_FA2019_A7
{
   ...
   {
      if(used == capacity)
         resize(size_type(1.05 * capacity) + 1);
   ...
   void p_queue::pop()
   {
      ...
      while(!is_leaf(iParent) && heap[iParent].priority <= big_child_priority(iParent))
      {
        iChild = big_child_index(iParent);
        swap_with_parent(big_child_index(iParent));
        iParent = iChild;
      }
   }
   ...
   //             used remains unchanged.
   {
      ...
      ItemType* tempHeap = new ItemType[new_capacity];
   ...
   //       than that of the other child, if there is one.)
   {
      ...
      size_type lhs_i = (2 * i) + 1;
      size_type rhs_i = (2 * i) + 2;
   ...
   // Post: The item at heap[i] has been swapped with its parent.
   {
      ...
      size_type parentI = parent_index(i);

      ItemType tItem = heap[parentI];
      heap[parentI] = heap[i];
      heap[i] = tItem;
   }
}
